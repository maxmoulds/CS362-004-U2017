...
int main() {
  ...
  int player2handcount = state.handCount[1];
  int player1handcount = state.handCount[0];
  ...
  asserttrue(state.handCount[1], player2handcount+1, "checking that player2 got a card");
  asserttrue(state.handCount[0], player1handcount+4, "checking that player1 got a card");
  ...
  cardEffect(card, choice1, choice2, choice3, &run2, handPos, &bonus);
  ...
  for (i = 0; i < ONE_TURN_GAMES; i++) {
    ...
    bonus = 0;
    handPos = 0;
    cardEffect(card, choice1, choice2, choice3, &one_turn_ref1, handPos, &bonus);
    bonus = 0;
    handPos = 0; /* shouldnt need to this but whatevs */
    cardEffect(card, choice1, choice2, choice3, &one_turn_ref2, handPos, &bonus);
