...
void testPlayGreatHall(void) {
    ...
    int kingdomCards[10] = { great_hall, adventurer, village, mine, council_room,
    ...
    // Add Great Hall card to player's hand
    int player = whoseTurn(&testState);
    int greatHallPos = testState.handCount[player];
    testState.hand[player][greatHallPos] = great_hall;
    ...
    // Set expected state based on Great Hall mechanics
    // Great Hall should draw one card, and increase actions by 1

    /*
     *  CASE 1: More than 1 card in deck
     */
    printf("Testing Great Hall mechanics with more than 1 card in deck.\n");
    ...
    // Great Hall should be placed in the played cards pile
    handCount = expectedState.handCount[player];
    playedCardCount = expectedState.playedCardCount;
    expectedState.playedCards[playedCardCount] = expectedState.hand[player][greatHallPos];
    expectedState.playedCardCount++;
    // Replace Great Hall with last card in hand
    expectedState.hand[player][greatHallPos] = expectedState.hand[player][handCount - 1];
    expectedState.handCount[player]--;

    // Use cardEffect to set testState
    cardEffect(great_hall, 0, 0, 0, &testState, greatHallPos, 0);
    ...
    /*
     *  CASE 2: Exactly 1 cards in deck
     */
    printf("\nTesting Great Hall mechanics with exactly 1 cards in deck.\n");

    // Set deck count to 1, discard count to 0
    player = 0;
    ...
    /*
     *  CASE 3: 0 cards in deck, 0 in discard (not testing shuffle)
     */
...
void runTestCases(struct gameState *expectedState, struct gameState *testState) {
    ...
    // TEST: Number of actions should have increased by 1
